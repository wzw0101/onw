...
export default function VotePhase({ roomInfo, playerId }: VotePhaseProps) {
    ...
    return (
        <div className="space-y-4">
            <p className="text-lg font-bold text-center">Vote Turn</p>
            <p className="text-sm text-base-content/60 mb-4">Choose a player to vote for:</p>
                ...
                {roomInfo.seats.map((seatPlayerId, index) => {
                    ...
                    return (
                        <div key={index} className={`p-4 rounded-lg border-2 cursor-pointer transition-colors text-center
                            ...
                            {isSelf && <div className="text-xs text-error">You</div>}
            ...
            <div className="flex gap-4">
                ...
                {roomInfo.hostPlayer === playerId && (
                    <button className="btn btn-secondary"
                        onClick={() => gameApi.endVoting(playerId)}>End Voting</button>
                )}
            </div>
        </div>
    );
